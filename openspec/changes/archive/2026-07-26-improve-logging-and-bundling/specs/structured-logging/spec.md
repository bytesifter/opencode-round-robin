...
event 层日志 SHALL 包含以下业务字段: `session`(sessionID 截短为前 4 位)、`model`(modelID)、`provider`(providerID)、`mode`(模式)、`agent`(agent 名)。SHALL NOT 记录完整 sessionID(隐私)。
...
- **AND** `session` 字段 SHALL 为 sessionID 的前 4 位(非完整值)
...
当使用 `logDir` 配置时,日志 SHALL 按日轮转,文件名格式为 `round-robin-YYYY-MM-DD.log`(本地时区)。旧日志文件 SHALL 保留,不自动删除。
...
插件 options SHALL 支持可选 `logDir` 字段(日志目录路径)。当 `logDir` 存在时,启用按日轮转模式。当 `logDir` 不存在但 `logPath` 存在时,使用 `logPath` 单文件模式(不轮转)。两者均不存在时使用默认路径(不轮转)。优先级: `logDir > logPath > 默认`。

#### Scenario: 配置 logDir 启用轮转

- **WHEN** `opencode.jsonc` 中配置 `"logDir": "/var/log/opencode"`
- **THEN** 日志 SHALL 写入 `/var/log/opencode/round-robin-YYYY-MM-DD.log`

#### Scenario: 配置 logPath 不轮转

- **WHEN** `opencode.jsonc` 中配置 `"logPath": "/tmp/rr.log"`(无 `logDir`)
- **THEN** 日志 SHALL 追加到 `/tmp/rr.log`(单文件,不轮转)

#### Scenario: 均未配置使用默认路径

- **WHEN** `opencode.jsonc` 中未配置 `logDir` 和 `logPath`
- **THEN** 日志 SHALL 追加到 `~/.local/share/opencode/round-robin.log`(单文件,不轮转)
